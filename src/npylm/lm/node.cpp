...
namespace npylm {
	namespace lm {
		...
		bool Node::parent_exists(){
			return !(_parent == NULL);
		}
		bool Node::child_exists(T token_id){
			return !(_children.find(token_id) == _children.end());
		}
		bool Node::need_to_remove_from_parent(){
			if(_parent == NULL){
				return false;
			}
		...
		int Node::get_num_tables_serving_word(T token_id){
			if(_arrangement.find(token_id) == _arrangement.end()){
				return 0;
			}
			return _arrangement[token_id].size();
		}
		int Node::get_num_customers_eating_word(T token_id){
			if(_arrangement.find(token_id) == _arrangement.end()){
				return 0;
			}
		...
		Node* Node::find_child_node(T token_id, bool generate_if_not_exist = false){
		...
		// 客をテーブルに追加
		bool Node::add_customer_to_table(T token_id, int table_k, double g0, vector<double> &d_m, vector<double> &theta_m, int &added_to_table_k_of_root){
		...
		bool Node::add_customer_to_table(T token_id, int table_k, double* parent_pw_at_depth, vector<double> &d_m, vector<double> &theta_m, int &added_to_table_k_of_root){
			auto itr = _arrangement.find(token_id);
			if(itr == _arrangement.end()){
		...
		bool Node::add_customer_to_new_table(T token_id, double g0, vector<double> &d_m, vector<double> &theta_m, int &added_to_table_k_of_root){
		...
		bool Node::add_customer_to_new_table(T token_id, double* parent_pw_at_depth, vector<double> &d_m, vector<double> &theta_m, int &added_to_table_k_of_root){
			_add_customer_to_new_table(token_id);
			if(_parent != NULL){
		...
		void Node::_add_customer_to_new_table(T token_id){
			auto itr = _arrangement.find(token_id);
			if(itr == _arrangement.end()){
				vector<int> tables = {1};
		...
		bool Node::remove_customer_from_table(T token_id, int table_k, int &removed_from_table_k_of_root){
			auto itr = _arrangement.find(token_id);
		...
		// 再帰計算があるので遅い
		bool Node::add_customer(T token_id, double g0, vector<double> &d_m, vector<double> &theta_m, bool update_beta_count, int &added_to_table_k_of_root){
		...
		bool Node::add_customer(T token_id, double* parent_pw_at_depth, vector<double> &d_m, vector<double> &theta_m, bool update_beta_count, int &added_to_table_k_of_root){
			init_hyperparameters_at_depth_if_needed(_depth, d_m, theta_m);
			double d_u = d_m[_depth];
			double theta_u = theta_m[_depth];
		...
		bool Node::remove_customer(T token_id, bool update_beta_count, int &removed_from_table_k_of_root){
			auto itr = _arrangement.find(token_id);
			assert(itr != _arrangement.end());
			vector<int> &num_customers_at_table = itr->second;
		...
		// 再帰計算が含まれるので遅い
		double Node::compute_Pw(T token_id, double g0, vector<double> &d_m, vector<double> &theta_m){
			init_hyperparameters_at_depth_if_needed(_depth, d_m, theta_m);
			double d_u = d_m[_depth];
			double theta_u = theta_m[_depth];
			double t_u = _num_tables;
		...
		// VPYLM
		double Node::stop_probability(double beta_stop, double beta_pass, bool recursive = true){
		...
		// VPYLM
		double Node::pass_probability(double beta_stop, double beta_pass, bool recursive = true){
		...
		// VPYLM
		void Node::increment_stop_count(){
			_stop_count++;
			if(_parent != NULL){
		...
		// VPYLM
		void Node::decrement_stop_count(){
			_stop_count--;
		...
		// VPYLM
		void Node::increment_pass_count(){
			_pass_count++;
			if(_parent != NULL){
				_parent->increment_pass_count();
			}
		...
		// VPYLM
		void Node::decrement_pass_count(){
			_pass_count--;
		...
		void Node::delete_child_node(T token_id){
		...
		int Node::get_max_depth(int base){
			int max_depth = base;
		...
		void Node::enumerate_nodes_at_depth(int depth, vector<Node*> &nodes){
			if(_depth == depth){
		...
		double Node::auxiliary_log_x_u(double theta_u){
			if(_num_customers >= 2){
		...
		double Node::auxiliary_y_ui(double d_u, double theta_u){
			if(_num_tables >= 2){
				double sum_y_ui = 0;
		...
		double Node::auxiliary_1_y_ui(double d_u, double theta_u){
			if(_num_tables >= 2){
				double sum_1_y_ui = 0;
				for(int i = 1;i <= _num_tables - 1;i++){
					double denominator = theta_u + d_u * i;
		...
		double Node::auxiliary_1_z_uwkj(double d_u){
			double sum_z_uwkj = 0;
			// c_u..
			for(auto elem: _arrangement){
				// c_uw.
		...
		void Node::init_hyperparameters_at_depth_if_needed(int depth, vector<double> &d_m, vector<double> &theta_m){
			if(depth >= d_m.size()){
		...
		void Node::serialize(Archive& archive, unsigned int version)
		{
			archive & _children;
